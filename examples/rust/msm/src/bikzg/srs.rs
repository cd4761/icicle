    ...
    G2Point: IsGroup + AsBytes,
{
    fn as_bytes(&self) -> Vec<u8> {
        ...
        let mut main_group_len_bytes: Vec<u8> = self
            .powers_main_group
            .len()
            .to_le_bytes()
            .to_vec();
...
fn add_usize(serialized_data: &mut Vec<u8>, data: usize) {
    let mut data_bytes: Vec<u8> = data
        .to_le_bytes()
        .to_vec();
    ...
    G2Point: IsGroup + Deserializable,
{
    fn deserialize(bytes: &[u8]) -> Result<Self, DeserializationError> {
        ...
        let x_dim_len = usize::try_from(x_dim_len_u64).map_err(|_| DeserializationError::PointerSizeError)?;
        ...
        let y_dim_len = usize::try_from(y_dim_len_u64).map_err(|_| DeserializationError::PointerSizeError)?;
        ...
        for i in 0..main_group_len {
            ...
            let point = G1Point::deserialize(
                bytes[i * size_g1_point + MAIN_GROUP_OFFSET..i * size_g1_point + size_g1_point + MAIN_GROUP_OFFSET]
                    .try_into()
                    .unwrap(),
            )?;
        ...
        for i in 0..3 {
            ...
            let point = G2Point::deserialize(
                bytes[i * size_g2_point + g2s_offset..i * size_g2_point + g2s_offset + size_g2_point]
                    .try_into()
                    .unwrap(),
            )?;
...
impl<const N: usize, F: IsPrimeField<RepresentativeType = UnsignedInteger<N>>, P: IsPairing> IsCommitmentScheme<F>
    for BivariateKateZaveruchaGoldberg<F, P>
{
    ...
    fn commit_bivariate(&self, bp: &BivariatePolynomial<FieldElement<F>>) -> Self::Commitment {
        ...
        msm(
            ...
            &self
                .srs
                .flatten_partitioned_g1_points(bp.x_degree, bp.y_degree),
        )
    ...
    fn commit_univariate(&self, p: &UnivariatePolynomial<FieldElement<F>>) -> Self::Commitment {
        ...
        let first_col_powers_main_group: Vec<_> = self
            ...
            .cloned()
            .collect();
        msm(&coefficients_y, &first_col_powers_main_group[..coefficients_y.len()])
            .expect("`points` is sliced by `cs`'s length")
    }
    ...
    fn verify(
        ...
        let g2 = &self
            .srs
            .powers_secondary_group[0];
        let tau_g2 = &self
            .srs
            .powers_secondary_group[1];
        let tetha_g2 = &self
            .srs
            .powers_secondary_group[2];
        ...
        for j in 0..col_len {
            row.push(
                y_row
                    .clone()
                    .mul(tau.pow(j)),
            ); // TODO check for not being wrong.
        }
        vec[i] = row;
    }
    vec
...
mod tests {
    ...
    fn create_srs(
    ) -> StructuredReferenceString<<BLS12381AtePairing as IsPairing>::G1Point, <BLS12381AtePairing as IsPairing>::G2Point>
    {
        let mut rng = rand::thread_rng();
        let tau_toxic_waste = FrElement::new(U256 {
            limbs: [rng.gen::<u64>(), rng.gen::<u64>(), rng.gen::<u64>(), rng.gen::<u64>()],
        });
        ...
        let powers_main_group: Vec<_> = g1_points_2d_vec
            .into_iter()
            .flatten()
            .collect();
