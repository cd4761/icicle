...
fn fr_elements_to_host_slice(elements: &[FrElement]) -> Vec<u8> {
    // Convert each FrElement to bytes and collect them into a single Vec<u8>
    let bytes: Vec<u8> = elements.iter().flat_map(|element| element.as_bytes()).collect();

    bytes
}

fn main() {
    // Create a BivariatePolynomial
    let bp = BivariatePolynomial::new(&[
        &[FrElement::from(1), FrElement::from(1)],
        &[FrElement::from(1), FrElement::from(1)],
    ]);

    // Flatten the polynomial to a 1D vector of FrElement
    let flattened_elements = bp.flatten_out();

    // Convert the flattened elements to bytes
    let bytes = fr_elements_to_host_slice(&flattened_elements);

    // Create a HostSlice from the bytes
    let host_slice: &HostSlice<u8> = HostSlice::from_slice(&bytes);

    println!("HostSlice: {:?}", host_slice);
}
